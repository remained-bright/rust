...
pub async fn boot(socket: &UdpSocket, connecting: &Cache<[u8; 6], ()>) {
  ...
  for (n, (_, li)) in TX
    .range::<u64, ByteVec, _>(db::time_ipv4, ..)
    .unwrap()
    .enumerate()
  {
    for ipv4 in li {
      let ipv4 = bytes_to_addr::v4(ipv4.0.try_into().unwrap());
      send!(ipv4);
    }
    if n == 128 {
      return;
  ...
  .split(' ')
  {
    match SocketAddrV4::from_str(ip) {
      Ok(v4) => {
        if let Some(_) = TX
          .one::<_, u64>(db::ipv4_time, &ByteVec(v4.to_bytes().to_vec()))
          .unwrap()
        {
          continue;
        }
